...
function Text(opts) {
  ...
  //material of text does not work with the geometry
  this.align = opts.align;
  //wrap count can control the size of the text

  this.wrapCount = opts.wrapCount;
  if (!('side' in opts)) {
    opts.side = 'front';
  };
  this.side = opts.side;
...
function setText(entity) {
  entity.tag.setAttribute('text', 'value:' + entity.value + '; align:' + entity.align + '; wrap-count:' + entity.wrapCount + '; side:' + entity.side + '; color:' + entity.color + '; letterSpacing:' + entity.letterSpacing);
}
...
function World(id) {
  ...
  this.getUserRotation = function() {
    return {
      x: this.camera.rotationX * 180 / Math.PI,
      y: this.camera.rotationY * 180 / Math.PI,
      z: this.camera.rotationZ * 180 / Math.PI
    };
  }
